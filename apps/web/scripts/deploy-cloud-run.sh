...
# Always operate from the Laravel app root (apps/web), regardless of cwd.
...
Usage (from anywhere):
  ...
  PROJECT_ID=my-gcp-project ./apps/web/scripts/deploy-cloud-run.sh
...
Cloud Build context is apps/web (this directory's Dockerfile).
EOF
}
...
echo "  # from monorepo root: PROJECT_ID=$PROJECT_ID ./apps/web/scripts/migrate-cloud-run.sh"
echo "Smoke-test checklist: docs/testing.md"
