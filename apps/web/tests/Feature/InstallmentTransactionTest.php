...
function installmentGeoCityId(): int
{
    return 1001;
}
...
it('creates twelve installment transactions linked to one group', function () {
    ...
    $result = app(TransactionService::class)->create($user, [
        ...
        'person_id' => $person->id,
        'geo_city_id' => installmentGeoCityId(),
        'status' => 'PAID',
    ]);
    ...
    expect($transactions->pluck('installment_group_id')->unique()->all())->toBe([$group->id])
        ...
        ->and($transactions->every(fn (Transaction $tx): bool => $tx->geo_city_id === installmentGeoCityId()))->toBeTrue()
...
it('rejects another users category or person', function () {
    $user = installmentUser();
...
it('allows external geo city ids without local ownership checks', function () {
    $user = installmentUser();
    ...
    expect(Transaction::query()->count())->toBe(2)
        ->and(Transaction::query()->value('geo_city_id'))->toBe(installmentGeoCityId());
});
