...
class TransactionResource extends Resource
{
    ...
    public static function getEloquentQuery(): Builder
    {
        return parent::getEloquentQuery()
            ->with(['category.parent', 'person', 'loan', 'installmentGroup'])
            ...
            ->components([
                ...
                Select::make('person_id')
                    ...
                    ->afterStateUpdated(function (?string $state, callable $set): void {
                        ...
                        if (! $person?->geo_city_id) {
                            return;
                        }

                        $location = app(GeoCityResolver::class)->formStateFromCityId($person->geo_city_id);
                        $set('geo_country_code', $location['geo_country_code']);
                        $set('geo_region_id', $location['geo_region_id']);
                        $set('geo_city_id', $location['geo_city_id']);
                    }),
            ...
            ->components([
                ...
                Section::make('Detalhes')
                    ...
                    ->visible(fn (Transaction $record): bool => filled($record->person?->name)
                        ...
                        || filled(self::formatCityDisplay($record))
            ...
            ->filters([
                ...
                SelectFilter::make('geo_city_id')
                    ...
                    ->query(function (Builder $query, array $data): void {
                        ...
                        $query->where('geo_city_id', $data['value']);
                    }),
    ...
    private static function mutateTransactionRecordDataForForm(array $data): array
    {
        ...
        $geoCityId = isset($data['geo_city_id']) && filled($data['geo_city_id'])
            ? (int) $data['geo_city_id']
            : null;
    ...
    public static function mutateTransactionFormDataForSave(array $data): array
    {
        ...
        unset($data['geo_country_code'], $data['geo_region_id']);
    ...
    public static function mobileCardData(Transaction $record): array
    {
        ...
        return [
            ...
            'person' => $person,
            'city' => self::formatCityDisplay($record),
    ...
    {
        $label = app(GeoPresenter::class)->cityRegionLabel($record->geo_city_id);

        if ($label === null || $label === GeoPresenter::UNAVAILABLE || $label === GeoPresenter::MISSING) {
            return $label;
        }
    ...
    /**
     * Distinct Geo city IDs already used by the authenticated user (filter options).
     *
     * @return array<int, string>
     */
    private static function cityOptions(): array
    {
        $ids = Transaction::query()
            ->where('user_id', Auth::id())
            ->whereNotNull('geo_city_id')
            ->distinct()
            ->orderBy('geo_city_id')
            ->pluck('geo_city_id')
            ->all();

        return app(GeoPresenter::class)->labelsForIds($ids);
    }
