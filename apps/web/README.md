# Finba.se (Laravel / Filament)

Laravel application for Finba.se. In the monorepo this package lives at **`apps/web`**.
...
```text
finba.se/
├── apps/web/   ← you are here (Laravel)
└── apps/geo/   # Go Geo API (geographic source of truth)
```

Work from this directory for Composer, Artisan, npm, Docker, and deploy scripts:
...
| [docs/changelog.md](docs/changelog.md) | Canonical changelog source, featured entries, visibility |
...
| [docs/geo.md](docs/geo.md) | Laravel client for the Geo API |

Public changelog: [https://app.finba.se/changelog](https://app.finba.se/changelog) (`route('changelog')`). Entries live in `resources/data/changelog.php`.
